...
class BlockHeader:
    ...
    timestamp: int # (8 bytes, uint64 big-endian)
    difficulty: int # (4 bytes, uint32 big-endian)
    nonce: int # (8 bytes, uint64 big-endian)
    ...
    def pack(self) -> bytes:
        ...
        return (self.prev_hash + self.txs_hash + u64_be(self.timestamp) +
